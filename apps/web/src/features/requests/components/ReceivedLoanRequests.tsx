              ...
              <article key={request.id} className="rounded-lg border p-4">
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  <div className="space-y-1">
                    <Link className="font-semibold hover:underline" href={routes.bookDetail(request.book.id)}>
                      {request.book.title}
                    </Link>
                ...
                <dl className="mt-4 space-y-2 text-sm">
                  <div>
                    ...
                    <dd>{request.requester.name ?? request.requester.email ?? t("dashboard.userFallback")}</dd>
                  </div>
                  {request.message ? (
                    <div>
                      <dt className="font-medium text-muted-foreground">
